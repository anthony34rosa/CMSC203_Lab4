   ...
   public void fillValues( )
   {
       ...
       for ( int r = 0; r < intArray.length; r++ )
       {
           ...
           for ( int c = 0; c < intArray[r].length; c++ )
           {
               ...
               int randomNumber = rand.nextInt( 31 ) + 50;
               // Assign the random number to the current element in the array
               intArray[r][c] = randomNumber;
               // Animate the algorithm by calling the animate method
               animate( r, c );  // needed to create visual feedback
           }
...
public void printArray() {
    ...
    for (int r = 0; r < intArray.length; r++) {
        ...
        for (int c = 0; c < intArray[r].length; c++) {
            ...
            // To animate the algorithm, put this method call as the last element in your inner for loop
            animate(r, c);
        }
        System.out.println(); // Move to the next line after printing each row
    }
   ...
   */
   public void setValues(int value, int row) {
	    for (int column = 0; column < intArray[row].length; column++) {
	        ...
	        // To animate the algorithm, put this method call as the last element in your for loop
	        animate(row, column);
	    }
	}
   ...
   */
   public int findMinimum(int column) {
	    // Initialize minimum with the first element of the column
	    ...
	    for (int r = 1; r < intArray.length; r++) {
	        ...
	        if (intArray[r][column] < minimum) {
	            // Update the minimum with the smaller value
	            minimum = intArray[r][column];
	        }

	        // To animate the algorithm, put this method call as the last element in your for loop
	        animate(r, column, minimum);
	    }
   ...
   public int countFound(int value) {
	    ...
	    for (int r = 0; r < intArray.length; r++) {
	        ...
	        for (int c = 0; c < intArray[r].length; c++) {
	            ...
	            if (intArray[r][c] == value) {
	                // Increment the count if the value is found
	                count++;
	            }
